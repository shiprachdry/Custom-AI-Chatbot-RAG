...
def RAG_response_api(context, question):
    review=f"Answer with empathy, care & compassion like a friend in less than 100 word count. Don't always start with 'I am really sorry', try to use different phrases for each response. Use only the content in pdf for answering, add links if provided. Give resources & tips in bullet points. Given below is the context and question of the user. context: {context} question:{question}. If the content is not in the context ,then say reach out to EAP at /go/eap or Health team at SAP (link). In the end, always check if there is anything else the user needs?"
...
loader = PyPDFLoader("C:\\Users\\I068117\\UT_Machine Learning\\Text-Analytics_NLP\\Sentiment Analysis\\Are you OK pager.pdf")
docs= loader.load()
...
# Set the app title 
    sl.title(' Mental Health Assist App')

    with sl.chat_message("assistant"):
     sl.write("Hello! 👋 I am your mental health assistant, how can I help you?")
